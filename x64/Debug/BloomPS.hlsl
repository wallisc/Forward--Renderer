...
#define BLUR_COUNT_SQRT 8
#define BLUR_COUNT (BLUR_COUNT_SQRT * BLUR_COUNT_SQRT) 
#define PI 3.14
#define E  2.72
#define STANDARD_DEVIATION 0.4
#define LUMA_CUTOFF 0.55
...
float4 main(VertexOutput input) : SV_TARGET
{
   ...
   for (int x = -(BLUR_COUNT_SQRT / 2); x < BLUR_COUNT_SQRT / 2 + 1; x++)
   {
      for (int y = -(BLUR_COUNT_SQRT / 2); y < BLUR_COUNT_SQRT / 2 + 1; y++)
      {
         ...
         //float factor = (1.0f / (STANDARD_DEVIATION * 2 * PI)) * pow(E, - (x * x + y * y) / (2 * STANDARD_DEVIATION * STANDARD_DEVIATION ));
         color += sampleColor * applyColor;
      }
   }
   color /= BLUR_COUNT;
