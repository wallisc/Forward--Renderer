...
void Renderer::Update(FLOAT dt, BOOL *keyInputArray) 
{
   ...
   XMMATRIX perspInv = XMMatrixInverse(&determinant, perspective);

   XMMATRIX invMvp = perspInv * lightViewInv;
   XMStoreFloat4x4(&m_psLightConstBuf.invMvp, invMvp);

   XMVECTOR testPoint = XMVectorSet(1, 1, 1, 1);
   XMVECTOR testPoint2 = XMVector4Transform(testPoint, m_vsLightTransConstBuf.mvp);
   XMVECTOR testPoint3 = XMVectorScale(testPoint2, 1.0f / XMVectorGetW(testPoint2));
   
   XMVECTOR testPoint4 = XMVector4Transform(testPoint3, invMvp);
   XMVECTOR testPoint5 = XMVectorScale(testPoint4, 1.0f / XMVectorGetW(testPoint4));
}
...
void Renderer::Render() 
{
   ...
   m_d3dContext->CSSetConstantBuffers(0 , 1, pFirstPassCbs);
   ...
   for (UINT draw = 0; draw < 2; draw++)
   {
      if (draw == 0)
      {
         ...
         m_d3dContext->OMSetRenderTargets(2, pLightMapRtv, m_pShadowMap->GetDepthStencilView());
      }
      else
      {
         ...
         ID3D11UnorderedAccessView *pLightCullingUav[] = { 
            ...
            m_pTileLightBuffer->GetUnorderedAccessView()
         };
         ...
         m_d3dContext->Dispatch(m_tileXCount, m_tileYCount, 1);
         ...
         ID3D11ShaderResourceView *pShadowSrv[] = {
            ...
            m_pTileLightBuffer->GetShaderResourceView(),
         ...
         m_d3dContext->PSSetShaderResources(1 , 3, pShadowSrv);
...
bool Renderer::LoadContent() 
{
   ...
   m_tileXCount = m_shadowMapWidth / TILED_WIDTH_DIVISIONS;
   m_tileYCount = m_shadowMapHeight / TILED_HEIGHT_DIVISIONS;
   UINT numTiles = m_tileXCount * m_tileYCount;
   UINT numLights = numTiles;

   m_pLightBuffer = new RWStructuredBuffer<PS_Point_Light>(m_d3dDevice, numTiles, true);
   m_pTileLightBuffer = new RWStructuredBuffer<PS_Point_Light>(m_d3dDevice, numTiles);
