...
bool D3DBase::Initialize(HINSTANCE hInstance, HWND hwnd) 
{
   ...
   swapChainDesc.BufferCount = NUM_SWAP_CHAIN_BUFFERS;
	...
	ID3D11Texture2D *m_backBufferResource;
	result = m_swapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)&m_backBufferResource);
	...
	result = m_d3dDevice->CreateRenderTargetView(m_backBufferResource, 0, &m_backBufferTarget);

	if (m_backBufferResource) m_backBufferResource->Release();
	if (FAILED(result))
	{
